...
const TimeLogs = () => {
  ...
  const filteredLogs = useMemo(() => {
    return timeLogs.filter((log) => {
      const matchSearch =
        ...
        log.production_name?.toLowerCase().includes(search.toLowerCase());
  ...
  }, [timeLogs, search, fromDate, toDate, projectFilter]);
  ...
  const totalBudget = filteredLogs.reduce(
    (sum, l) => sum + timeToMinutes(l.time_budget_snapshot),
    0
  );

  const totalWork = filteredLogs.reduce(
          ...
          <div className="row g-2 mb-3 align-items-center">
            ...
            <div className="col-md-4">
              ...
              >
                ...
                {[...new Set(timeLogs.map(l => l.project_name).filter(Boolean))]
                  .map((name) => (
              ...
              <tbody>
                {loading ? (
                  ...
                  filteredLogs.map((log) => (
                    <tr key={log.id}>
                      <td className="fw-semibold">{log.JobID}</td>
                      <td>{log.project_name || "-"}</td>
                      <td>{log.employee_name}</td>
                      <td>{formatDDMMYYYY(log.date)}</td>
                      <td className="text-end">{formatTime(log.time_budget_snapshot)}</td>
                      <td className="text-end">{formatTime(log.time)}</td>
                      <td className="text-end">{formatTime(log.overtime)}</td>
                      <td className="text-end">{formatTime(log.total_time)}</td>
                    </tr>
                  ))
                )}
              </tbody>

              {/* ✅ TOTAL ROW (INCLUDING TIME BUDGET) */}
              <tfoot>
